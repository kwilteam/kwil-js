...
import { concatBytes, numberToUint16BigEndian } from "../utils/bytes"
import { booleanToBytes, stringToBytes } from "../utils/serial"
...
function encodeActionCall(ac: ActionCall): string {
    const acVersion = 0;
...
function numberToUint32LittleEndian(number: number) {
...
function numberToUint32BigEndian(number: number) {
    if (number < 0 || number > 0xFFFFFFFF || !Number.isInteger(number)) {
      throw new RangeError("The number must be an integer between 0 and 4294967295.");
    }
