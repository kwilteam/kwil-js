...
const createEvent = async (_eventName, _eventDesc, _eventTime, _country, _city, _tags, _group, _username, _privateJWK) => {
    const _privateKey = rs.KEYUTIL.getKey(_privateJWK)
    const _publicKey = getPublicJWKFromPrivateJWK(_privateJWK)
    const event = new Event(_eventName, _eventDesc, _eventTime, _country, _city, _tags, _group, _username.toLowerCase(), _publicKey)
    ...
    const params = {
        ...
        timeout: 20000,
        data: {
            data: event, signature: dataSignature
        }
    }
    await axios(params)
    return event
